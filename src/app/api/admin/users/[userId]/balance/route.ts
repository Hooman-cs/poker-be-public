/**
 * @fileoverview Admin User Balance API
 * Adds or removes locked bonus from a user wallet.
 */
...
export async function POST(
  ...
  try {
    ...
    if (action === 'remove' && wallet.lockedBonus < bonusAmount) {
      return NextResponse.json(
        { message: 'Insufficient locked bonus balance' },
        { status: 400 }
      );
    }

    const balanceChange = action === 'add' ? bonusAmount : -bonusAmount;
    wallet.lockedBonus += balanceChange;
    await wallet.save();
    ...
    console.error(`[Admin Balance Update Error - ${params.userId}]:`, error.message);
    return NextResponse.json(
